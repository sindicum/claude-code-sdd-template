...
2. featureブランチを作成する:
   ```bash
   git checkout -b feature/[機能名のkebab-case]
   ```
3. `Task`ツールで `git-operator` サブエージェントをモード2（コミット）で起動する:
   - `subagent_type`: "git-operator"
   - `description`: "Git commit for [機能名]"
   - `prompt`: "モード2（コミット）で実行してください。現在のブランチは feature/[機能名のkebab-case] です。変更内容を分析し、Conventional Commits形式のコミットメッセージを生成してユーザーに提示してください。ユーザーの承認を得てからコミットを実行してください。"
...
- ステップ9: 変更がfeatureブランチにコミットされている。
