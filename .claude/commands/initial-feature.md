...
**このステップはユーザーの確認を得てからコミットする。自動コミットは行わない。**

1. developブランチの存在を確認する（存在しなければ作成）
2. developブランチに直接コミットする（初回実装のためfeatureブランチは作成しない）:
   ```bash
   git checkout develop
   ```
3. `Task`ツールで `git-operator` サブエージェントをモード2（コミット）で起動する:
   - `subagent_type`: "git-operator"
   - `description`: "Git commit for initial implementation"
   - `prompt`: "モード2（コミット）で実行してください。現在のブランチは develop です。初回実装の変更内容を分析し、Conventional Commits形式のコミットメッセージを生成してユーザーに提示してください。ユーザーの承認を得てからコミットを実行してください。"
4. ユーザー承認後にコミットが実行される
5. 結果を報告する

**注意:** このステップは「完全自動実行」原則の例外です。コミット承認のみユーザーの介入が必要です。
...
- ステップ9: 変更がdevelopブランチにコミットされている。
