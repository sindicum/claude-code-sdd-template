...
## フェーズ0: プロジェクトスキャフォールド（初回実装時のみ）

> **注意**: このフェーズは `/initial-feature` コマンドによる初回実装時にのみ使用します。
> `/add-feature` による機能追加時はこのフェーズを削除してください。

- [ ] ディレクトリ構造作成（repository-structure.mdに基づく）
- [ ] package.json作成（architecture.mdの技術スタックに基づく）
...
- [ ] ビルド・テスト基盤の動作確認
  ...
  - [ ] `npm test` が実行可能であることを確認
  - [ ] `npm run lint` が実行可能であることを確認
