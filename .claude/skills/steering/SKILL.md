...
1. **ステアリングディレクトリの確認**
   ...
   ```
   1. `.steering/` 内の既存ディレクトリを走査し、最大の通し番号を取得
   2. 最大番号+1を3桁ゼロ埋めで採番（初回は001）
   3. `.steering/[NNN]_[YYYYMMDD]_[機能名]/` の形式でディレクトリを作成
      例: .steering/001_20250115_add-user-profile/
   ```

2. **初回実装の判定**

   ```
   Glob('src/**/*.{ts,tsx,js,jsx}') でソースコード有無を確認
   - ソースコードが存在しない → 初回実装（フェーズ0テンプレートを含む、design.mdに初期構造セクションを含む）
   - ソースコードが存在する → 機能追加（フェーズ0を除外、design.mdの初期構造セクションを除外）
   ```
...
4. **テンプレートからファイル作成**
   ...
   - `.claude/skills/steering/templates/requirements.md` → `.steering/[NNN]_[日付]_[機能名]/requirements.md`
   - `.claude/skills/steering/templates/design.md` → `.steering/[NNN]_[日付]_[機能名]/design.md`
   - `.claude/skills/steering/templates/tasklist.md` → `.steering/[NNN]_[日付]_[機能名]/tasklist.md`

   **初回実装の場合**: design.mdの「プロジェクト初期構造」セクションを残し、tasklist.mdの「フェーズ0」を残す
   **機能追加の場合**: design.mdの「プロジェクト初期構造」セクションを削除し、tasklist.mdの「フェーズ0」を削除する
...
6. **テストタスクの検証（必須）**
   ...
   tasklist.md作成後、以下を検証する:

   **チェックリスト**:
   - [ ] 各フェーズに「テストファイル作成」タスクが含まれているか？
   - [ ] テストファイルのパスが具体的に記載されているか？（例: `src/lib/__tests__/url.test.ts`）
   - [ ] 「手動動作確認」だけでテストタスクを構成していないか？
   - [ ] テスト免除がある場合、技術的理由が明記されているか？

   **テスト対象の判定基準**:
   - ユーティリティ関数 → 必須（例: `tests/unit/lib/url.test.ts`）
   - サービス層 → 必須（例: `tests/unit/services/TaskService.test.ts`）
   - カスタムフック（ロジック系）→ 推奨
   - UIコンポーネント（ブラウザランタイム依存等）→ 免除可（理由明記）

   **禁止パターン**:
   - 「テストファイル作成」を「手動動作確認」に置換してはならない
   - `npm run typecheck` や `npm run lint` をテストの代替としてはならない
   - これらは品質チェックフェーズの項目であり、テストタスクではない
...
- TaskCreate/TaskUpdateツールだけで進捗管理する（タスクツールは補助、tasklist.mdが正式）
...
```
Read('.steering/[NNN]_[日付]_[機能名]/tasklist.md')
```
...
3. **ユーザーに報告**
   ...
   ```
   **注意**: `/add-feature` コマンド経由で実行されている場合は、報告のみ行い停止しない。次のフェーズに自動的に進むこと。
...
- TaskCreate/TaskUpdateは揮発的なメモ（ユーザーには見えない）
