...
1. ドキュメント作成: 永続ドキュメント(`docs/`)で「何を作るか」「どう作るか（基本設計）」を定義
...
1. このテンプレートを使用
2. `/setup-project` で永続的ドキュメント作成(対話的に 6 つ作成)
3. `/initial-feature` で初回実装（P0機能全てをスキャフォールドから実装）
4. `/add-feature [機能]` で機能追加（2回目以降の機能実装）
...
# Git操作
> /commit                           # 変更をコミット
> /commit feat(auth): ログイン追加   # メッセージ指定でコミット
```
